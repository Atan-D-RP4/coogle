...
int compare_cursors(const void *a, const void *b) {
	...
	size_t rank = levenstein_distance(normalized_query, clang_getCString(sig_a)) - 
		levenstein_distance(normalized_query, clang_getCString(sig_b));
...
int main(int argc, char *argv[]) {
	...
	qsort(children.items, children.count, sizeof(CXCursor), compare_cursors);
